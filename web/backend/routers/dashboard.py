...
def calculate_expected_position(team_id: int, period: str, exclude_ir: bool, league_meta):
    """
    Рассчитывает ожидаемую позицию команды в конце чемпионата на основе:
    - Текущих результатов матчапов
    - Расписания будущих матчапов
    - Симуляции по статистике (avg)
    
    Returns:
        int или None - ожидаемая позиция команды в конце сезона
    """
    try:
        ...
        # Инициализируем результаты для всех команд
        team_records = {tid: {'wins': 0, 'losses': 0, 'ties': 0} for tid in all_team_ids}
        
        # Получаем текущие результаты из реальных матчапов (до current_week)
        for week in range(1, current_week):
            matchups = league_meta.get_matchups_for_week(week)
            if not matchups:
                continue
            ...
            for matchup in matchups:
                ...
                matchup_summary = league_meta.get_matchup_summary(week, team1_id, team2_id)
                if not matchup_summary:
                    continue
                
                team1_wins = matchup_summary['team1_wins']
                team2_wins = matchup_summary['team2_wins']
                ties = matchup_summary['ties']
                
                # Обновляем результаты команд
                if team1_wins > team2_wins:
                    ...
                    # Ничья в матчапе - обе команды получают по 1 ничьей
                    team_records[team1_id]['ties'] += 1
                    team_records[team2_id]['ties'] += 1
        
        # Определяем общее количество недель (пробуем получать матчапы для будущих недель)
        max_week = current_week
        for test_week in range(current_week + 1, current_week + 30):  # Максимум 30 недель вперед
            test_matchups = league_meta.get_matchups_for_week(test_week)
            if test_matchups:
                max_week = test_week
            else:
                break
        
        # Если нет будущих недель, возвращаем текущую позицию
        if max_week <= current_week:
            # Рассчитываем текущую позицию
            teams_with_records = []
            for tid in all_team_ids:
                wins = team_records[tid]['wins']
                losses = team_records[tid]['losses']
                ties = team_records[tid]['ties']
        ...
        # Получаем статистику команд по режиму team_stats_avg для симуляции будущих матчапов
        ...
        # Симулируем будущие матчапы (от current_week + 1 до max_week)
        for week in range(current_week + 1, max_week + 1):
            matchups = league_meta.get_matchups_for_week(week)
            if not matchups:
                continue
            ...
            for matchup in matchups:
                ...
                # Проверяем, что у обеих команд есть статистика
        ...
        # Рассчитываем финальную позицию на основе итоговых результатов
        teams_with_records = []
        for tid in all_team_ids:
            wins = team_records[tid]['wins']
            losses = team_records[tid]['losses']
        ...
        print(f"Error calculating expected position: {e}")
        import traceback
        traceback.print_exc()
        return None
...
def get_dashboard(
    ...
    # Рассчитываем ожидаемую позицию в конце чемпионата
    expected_position = calculate_expected_position(team_id, period, exclude_ir, league_meta)
