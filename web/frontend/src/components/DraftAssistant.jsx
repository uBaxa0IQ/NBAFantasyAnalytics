...
<span className="block text-xl leading-5" aria-hidden="true">⚙</span>
            ...
            {!mainTeam ? (
                ...
                <button disabled={!player.player_id || plan.queue.some(p => p.player_id === player.player_id)} className="rounded border px-2 disabled:opacity-40" onClick={() => updatePlan({ queue: [...plan.queue, player] })}>В очередь</button>
                ...
                {activeTab === 'draft' && !isUpcoming && !waitingForFirstLiveSnapshot && (!officialDraftMode || hasLiveSnapshot) && (
                    <div className="space-y-4">
                        ...
                        <AdaptiveStrategyPanel strategy={recommendations.adaptive_strategy} />
