...
<span className="block text-xl leading-5" aria-hidden="true">⚙</span>
            ...
            {!isPostDraft && <section className="mb-4 rounded border bg-white p-4">
                ...
                <p className="text-xs text-gray-500">Сохраняется для этой лиги, сезона и команды. Пики отправляются только через ESPN.</p>
                ...
                {!plan.queue.length && <p className="mt-2 text-sm text-gray-500">Добавьте игроков из таблицы ниже.</p>}
                {isUpcoming && <div className="mt-4 border-t pt-3"><h2 className="font-semibold">Виртуальный состав · {plan.mock.length}</h2>
                    <p className="text-xs text-gray-500">Выбранные игроки участвуют в пересчёте рекомендаций и симуляции.</p>
                    ...
                    {!!plan.mock.length && <button className="mt-2 text-sm text-red-700" onClick={() => updatePlan({ mock: [] })}>Очистить состав</button>}
                </div>}
            </section>}
            ...
            {!mainTeam ? (
                ...
                {activeTab === 'players' && !waitingForFirstLiveSnapshot && (!officialDraftMode || hasLiveSnapshot) && (
                    <section className="overflow-hidden rounded-xl border bg-white">
                        ...
                        {!isPostDraft && effectivePlayersView === 'stats' && <div className="border-b bg-blue-50 px-4 py-3 text-sm text-blue-900">Нейтральный статистический список доступных игроков: без ADP, вероятности доступности, очереди и draft-рекомендаций.</div>}
                        {effectivePlayersView === 'draft' && <div className="border-b bg-blue-50 px-4 py-3 text-sm text-blue-900">Только доступные игроки по текущему снимку драфта{pickCount ? ` · после пика #${pickCount}` : ''}{draftedIds.size ? ` · скрыто выбранных: ${draftedIds.size}` : ''}.</div>}
                        <div className="overflow-x-auto"><table className="min-w-full border-collapse bg-white text-sm"><thead><tr className="bg-gray-100">
                            ...
                            {effectivePlayersView === 'draft' && <th onClick={() => handleSort('espn_market_pick')} className="cursor-pointer whitespace-nowrap border p-2">Рынок ESPN<SortIcon column="espn_market_pick" /></th>}<th onClick={() => handleSort('total_z')} className="cursor-pointer whitespace-nowrap border p-2">Total Z<SortIcon column="total_z" /></th>
                ...
                {isUpcoming && <button disabled={!player.player_id || recommendationsLoading || roster.length >= rosterLimit || plan.mock.some(p => p.player_id === player.player_id)} className="rounded border px-2 disabled:opacity-40" onClick={() => updatePlan({ mock: [...plan.mock, player] })}>В состав</button>}
                            ...
                            {effectivePlayersView === 'draft' && <td className="border p-2 text-center"><div className="font-medium">#{player.espn_market_pick?.toFixed(1) || '—'}</div><div className="text-[11px] text-gray-400">Roto #{player.espn_roto_rank ?? '—'} · ADP {player.espn_adp?.toFixed(1) || '—'}</div>{player.availability_probability != null && <div className="text-xs text-blue-500">оценка {player.availability_probability}% · {player.draft_action}</div>}</td>}<td className={`border p-2 text-center font-bold ${zScoreTone(player.total_z)}`}>{Number(player.total_z || 0).toFixed(2)}</td>
                            {CATEGORIES.map(category => { const value = effectivePlayersView === 'stats' ? player.stats?.[category] : player.z_scores?.[category]; const zScore = Number(player.z_scores?.[category] || 0); const tone = effectivePlayersView === 'stats' ? zScoreTone(zScore) : (value > 0 ? 'text-green-600' : value < 0 ? 'text-red-600' : 'text-gray-400'); return <td key={category} className={`border p-2 text-center ${tone} ${effectivePlayersView === 'draft' && puntCategories.includes(category) ? 'opacity-30' : ''}`}>{effectivePlayersView === 'stats' ? <><div className="font-medium">{formatStat(category, value)}</div><div className="mt-0.5 text-[11px] opacity-75">Z {zScore > 0 ? '+' : ''}{zScore.toFixed(2)}</div></> : Number(value || 0).toFixed(2)}</td>; })}
                        </tr>)}</tbody></table></div>
                    </section>
                )}
                ...
                {activeTab === 'draft' && !isUpcoming && !waitingForFirstLiveSnapshot && (!officialDraftMode || hasLiveSnapshot) && (
                    <div className="space-y-4">
                        ...
                        {isPostDraft ? <section className="grid grid-cols-2 gap-3 lg:grid-cols-4"><MetricCard label="Место по силе" value={balancedComparison ? `#${balancedComparison.league_rank} / ${balancedComparison.team_count}` : '—'} hint="Сравнение полных составов" /><MetricCard label="Ожидаемо категорий" value={balancedComparison?.average_category_wins != null ? `${balancedComparison.average_category_wins.toFixed(2)} / ${CATEGORIES.length}` : '—'} hint="Средний H2H-профиль" /><MetricCard label="Лучшие категории" value={strongestCategories.length ? strongestCategories.join(' · ') : '—'} hint={strongestCategories.map(category => `${category} #${categoryRanks[category]}`).join(' · ')} tone="text-green-600" /><MetricCard label="Приоритет усиления" value={weakestCategories.length ? weakestCategories.join(' · ') : '—'} hint={weakestCategories.map(category => `${category} #${categoryRanks[category]}`).join(' · ')} tone="text-red-600" /></section> : <section className="grid grid-cols-2 gap-3 lg:grid-cols-5"><MetricCard label="Сила состава" value={rosterTotalZ.toFixed(1)} hint="Абсолютный Total Z" tone={rosterTotalZ >= 0 ? 'text-green-600' : 'text-red-600'} /><MetricCard label="Относительно лиги" value={balancedComparison ? `#${balancedComparison.league_rank} / ${balancedComparison.team_count}` : '—'} hint={completedRounds ? `После ${completedRounds}-го раунда` : 'После полного 1-го раунда'} /><MetricCard label="Категорий / матчап" value={selectedSimulation?.average_category_wins != null ? `${selectedSimulation.average_category_wins.toFixed(2)} / ${CATEGORIES.length}` : '—'} hint="Все категории лиги, включая punt" /><MetricCard label="Прогноз места" value={selectedSimulation?.average_league_rank != null ? `${selectedSimulation.average_league_rank.toFixed(1)} / ${teamCount}` : '—'} hint="По draft simulation" /><MetricCard label="Top-N по силе" value={selectedSimulation?.projected_top_n_strength_rate != null ? `${selectedSimulation.projected_top_n_strength_rate}%` : '—'} hint={`Топ-4 по силе: ${selectedSimulation?.top_four_strength_rate ?? '—'}%`} /></section>}
...
const AdaptiveStrategyPanel = ({ strategy }) => {
    ...
    return <section className="rounded-xl border bg-white p-4 shadow-sm"><div className="mb-3"><h2 className="font-bold">{fixed ? 'Ручной пант зафиксирован' : 'Веса стратегий модели'}</h2><p className="text-xs text-gray-500">{fixed ? 'Категории из настроек имеют нулевой вес при выборе; модель не переключит их автоматически' : 'Панты в настройках не выбраны: эвристические веса направлений обновляются по составу; проценты не являются вероятностью успеха'}</p></div><div className="flex flex-wrap gap-2">{rows.slice(0, 5).map(row => <div key={row.id} className="min-w-36 flex-1 rounded-lg border p-3"><div className="text-xs text-gray-500">{row.punt_categories.length ? `Punt ${row.punt_categories.join(' + ')}` : 'Balanced'}</div><div className="mt-1 text-lg font-bold text-blue-700">{Number(row.probability).toFixed(0)}%</div></div>)}</div></section>;
};
        ...
        <div className={`grid gap-3 p-4 sm:grid-cols-[2rem_1fr_auto] sm:items-center ${featured ? 'bg-blue-50' : 'hover:bg-gray-50'}`}>
            ...
            <div>
                ...
                <div className="mt-2 flex flex-wrap gap-1">
                    ...
                    {featured && player.decision_confidence != null && <span className={`rounded px-2 py-0.5 text-xs ${player.decision_label === 'решение устойчивое' ? 'bg-emerald-100 text-emerald-800' : player.decision_label === 'умеренное преимущество' ? 'bg-amber-100 text-amber-800' : 'bg-gray-100 text-gray-700'}`}>{player.decision_label} · лучше альтернативы в {Number(player.decision_confidence).toFixed(0)}% симуляций</span>}
                </div>
                {featured && player.decision_delta_ci95?.length === 2 && <div className="mt-2 text-xs text-gray-500">Против {player.decision_over}: {Number(player.decision_delta) >= 0 ? '+' : ''}{Number(player.decision_delta).toFixed(2)} категории · 95% интервал [{Number(player.decision_delta_ci95[0]).toFixed(2)}; {Number(player.decision_delta_ci95[1]).toFixed(2)}]</div>}
            </div>
...
const RosterZTable = ({ roster, puntCategories, onPlayerClick }) => {
    ...
    return <section className="overflow-hidden rounded-xl border bg-white shadow-sm">
        <div className="flex items-center justify-between border-b p-4"><h2 className="font-bold">Z-score игроков состава</h2><span className="text-sm text-gray-400">{roster.length}</span></div>
...
const SimulationView = ({ recommendations, selectedSimulation, simulationResults, rankedSimulationSlots, selectedSlotRank, effectiveSimulationRuns, simulationLoading, setSimulationSlot, puntCategories, teamCount, onPlayerClick, isPostDraft, benchmark, benchmarkLoading, benchmarkError, runBenchmark }) => {
    ...
    return <div className="space-y-4"><section className="rounded border bg-white p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><div className="font-bold">{isPostDraft ? `Итоговая симуляция позиции ${selectedSimulation.slot}` : `Позиция ${selectedSimulation.slot}`}</div><div className="text-sm text-gray-500">{recommendations.simulation.mode === 'all_slots' ? `#${selectedSlotRank} из ${rankedSimulationSlots.length} · ${effectiveSimulationRuns} прогонов` : `Позиция определена · ${effectiveSimulationRuns} прогонов`}{simulationLoading ? ' · уточняем…' : ''}</div></div>{recommendations.simulation.mode === 'all_slots' && <div className="flex flex-wrap gap-1">{simulationResults.map(result => <button key={result.slot} onClick={() => setSimulationSlot(result.slot)} className={`h-9 w-9 rounded border text-sm font-medium ${selectedSimulation.slot === result.slot ? 'border-blue-600 bg-blue-600 text-white' : 'bg-white text-gray-600'}`}>{result.slot}</button>)}</div>}</div>{!isPostDraft && selectedSimulation.round_targets?.length > 0 && <div className="mt-4 text-sm text-gray-600">Пики: <span className="font-medium text-gray-900">{selectedSimulation.round_targets.map(round => round.pick).join(' · ')}</span></div>}<div className="mt-2 text-sm"><span className="text-gray-500">Стратегия: </span><span className="font-medium">{puntCategories.length ? `Punt ${puntCategories.join(' + ')}` : 'без панта'}</span></div></section><section className="grid grid-cols-2 gap-2 md:grid-cols-5"><MetricCard label="Категорий / матчап" value={`${selectedSimulation.average_category_wins?.toFixed(2) || '—'} / ${activeCategoryCount}`} hint="Punt-категории тоже считаются" /><MetricCard label="Место" value={`${selectedSimulation.average_league_rank?.toFixed(1) || '—'} / ${teamCount}`} /><MetricCard label="Топ-4 по силе" value={`${selectedSimulation.top_four_strength_rate ?? selectedSimulation.top_four_probability ?? '—'}%`} /><MetricCard label="Top-N по силе" value={`${selectedSimulation.projected_top_n_strength_rate ?? selectedSimulation.playoff_probability ?? '—'}%`} hint="Доля симуляций, не вероятность плей-офф" /><MetricCard label="Средний GP" value={selectedSimulation.average_games_played ?? '—'} hint={`${selectedSimulation.average_low_gp_count ?? '—'} игроков < 50`} /></section>{!isPostDraft && <SimulationRosterTable recommendations={recommendations} simulation={selectedSimulation} onPlayerClick={onPlayerClick} />}<BenchmarkPanel benchmark={benchmark} loading={benchmarkLoading} error={benchmarkError} onRun={runBenchmark} disabled={isPostDraft} /><section className="rounded border bg-white p-4"><div className="mb-3 flex items-center justify-between gap-3"><h2 className="font-bold">Категории против лиги</h2><div className="text-sm text-gray-500">В среднем <span className="font-semibold text-gray-900">#{selectedSimulation.average_league_rank?.toFixed(1) || '—'} из {teamCount}</span></div></div><div className="flex flex-wrap justify-center gap-2">{CATEGORIES.map(category => { const rank = selectedSimulation.category_ranks?.[category]; const margin = selectedSimulation.category_margin?.[category]; return <div key={category} style={balancedCategoryCardStyle(CATEGORIES.length)} className={`rounded border p-3 ${puntCategories.includes(category) ? 'opacity-40' : ''}`}><div className="text-xs text-gray-500">{category} · #{rank ?? '—'}</div><div className={`font-bold ${margin > 0 ? 'text-green-600' : margin < 0 ? 'text-red-600' : 'text-gray-500'}`}>{margin != null ? `${margin > 0 ? '+' : ''}${margin.toFixed(1)}` : '—'}</div></div>; })}</div></section></div>;
};
...
const BenchmarkPanel = ({ benchmark, loading, error, onRun, disabled = false }) => {
    ...
    return <section className="overflow-hidden rounded border bg-white"><div className="flex flex-wrap items-center justify-between gap-3 border-b p-4"><div><h2 className="font-bold">ROTO против нашей модели</h2><p className="text-xs text-gray-500">Парные драфты · ESPN projected stats × GP · итог по всем категориям</p></div><button type="button" disabled={loading || disabled} onClick={onRun} className="rounded bg-blue-600 px-4 py-2 text-sm font-medium text-white disabled:cursor-not-allowed disabled:opacity-50">{loading ? 'Сравниваем…' : benchmark ? 'Пересчитать' : 'Запустить сравнение'}</button></div>{error && <div className="bg-red-50 p-4 text-sm text-red-700">{error}</div>}{benchmark && <><div className="grid gap-3 p-4 md:grid-cols-3">{benchmark.strategies.map(strategy => { const comparison = comparisonByStrategy[strategy.id]; const delta = comparison?.delta_category_wins; return <div key={strategy.id} className="rounded border p-4"><div className="font-semibold">{strategy.label}</div><div className="mt-2 text-2xl font-bold">{strategy.average_category_wins.toFixed(2)} / {benchmark.categories.length}</div><div className="text-xs text-gray-500">место #{strategy.average_league_rank.toFixed(2)} · top-4 {strategy.top_four_rate}%</div>{comparison && <div className={`mt-2 text-sm font-medium ${delta > 0 ? 'text-green-600' : delta < 0 ? 'text-red-600' : 'text-gray-600'}`}>{delta > 0 ? '+' : ''}{delta.toFixed(3)} кат. к ROTO · лучше в {comparison.better_rate}%</div>}</div>; })}</div><div className="border-t px-4 py-3 text-xs text-gray-500">{benchmark.total_paired_scenarios} одинаковых сценариев · соперники следуют ROTO с рыночным разбросом. Это projected benchmark, а не историческое доказательство преимущества.</div></>}</section>;
};
