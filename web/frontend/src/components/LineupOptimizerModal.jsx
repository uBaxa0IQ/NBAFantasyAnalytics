...
const LineupOptimizerModal = ({ teamId, onClose, puntCategories = [], period = '2026_total' }) => {
    ...
    useEffect(() => {
        ...
        api.get(`/lineup/${teamId}/optimize`, {
            ...
            .catch(err => {
                ...
                setError(err.response?.data?.error || 'Ошибка при оптимизации состава');
                setLoading(false);
            });
    }, [teamId, puntCategories]);
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 overflow-y-auto" onClick={onClose}>
            <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 my-8" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-4">
                    <h2 className="text-xl font-bold">Игроки по ценности</h2>
                ...
                {selectedPlayer && (
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setSelectedPlayer(null)}>
                        <div className="bg-white rounded-lg p-6 max-w-2xl w-full mx-4" onClick={e => e.stopPropagation()}>
                            <div className="flex justify-between items-center mb-4">
