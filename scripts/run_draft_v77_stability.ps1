...
if (-not $Execute) { python scripts/draft_v77_stability.py; exit $LASTEXITCODE }
$lateOutput = 'E:\NBAFantasyAnalytics\artifacts\draft_ml\standard8-v77-stability'
...
$lateEntry = @('-u','scripts/draft_v77_stability.py','--execute')
...
Write-Host "STABILITY CHECK. Worker PID: $($lateProcess.Id). Two training repeats, then evaluation; no universal-model launch."
