...
if (-not $Execute) { python scripts/draft_v74_late_search.py; exit $LASTEXITCODE }
$lateOutput = 'E:\NBAFantasyAnalytics\artifacts\draft_ml\standard8-v74-late-search'
...
$lateEntry = if (Test-Path "$lateOutput\status.json") { @('-u','scripts/run_v74_resilient.py') } else { @('-u','scripts/draft_v74_late_search.py','--execute') }
...
Write-Host "LATE SEARCH TEST ONLY. Worker PID: $($lateProcess.Id). No training."
