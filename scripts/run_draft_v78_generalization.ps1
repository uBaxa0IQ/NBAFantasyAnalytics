...
if (-not $Execute) { python scripts/draft_v78_generalization.py; exit $LASTEXITCODE }
$runOutput = 'E:\NBAFantasyAnalytics\artifacts\draft_ml\standard8-v78-generalization'
...
$worker = Start-Process -FilePath $pythonExe -ArgumentList @('-u','scripts/draft_v78_generalization.py','--execute') `
...
Write-Host "V7.8 GENERALIZATION. Worker PID: $($worker.Id). Viewer may be closed without stopping it."
Write-Host 'Generate -> GPU train -> validation gate -> frozen holdout. No promotion or universal-model launch.'
...
if ($null -ne $status -and $status.pid -eq $worker.Id -and $status.phase -eq 'COMPLETE' -and $status.completed -eq $status.total) {
    Write-Host 'COMPLETE. Results saved; stopped for user review.' -ForegroundColor Green
} elseif ($null -ne $status -and $status.pid -eq $worker.Id -and $status.phase -eq 'STOP_FOR_REVIEW') {
    Write-Host 'STOPPED AT VALIDATION GATE. Holdout was not opened.' -ForegroundColor Yellow
} else {
