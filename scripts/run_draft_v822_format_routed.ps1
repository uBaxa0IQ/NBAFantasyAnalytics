...
if (-not $Execute) {
    python scripts/draft_v822_format_routed.py
    exit $LASTEXITCODE
}

$runOutput = 'E:\NBAFantasyAnalytics\artifacts\draft_ml\v822-format-routed'
...
$existing = Get-CimInstance Win32_Process | Where-Object {
    ...
    ($_.CommandLine -like '*draft_v822_format_routed.py*--execute*')
}
if ($existing) {
    throw "V8.2.2 already running: $($existing.ProcessId -join ', ')"
}
...
$worker = Start-Process -FilePath $pythonExe `
    -ArgumentList @('-u', 'scripts/draft_v822_format_routed.py', '--execute') `
    -WorkingDirectory 'E:\NBAFantasyAnalytics' `
    -WindowStyle Hidden `
    -RedirectStandardOutput "$runOutput\out-$stamp.log" `
    -RedirectStandardError $errorLog `
    -PassThru
$workerHandle = $worker.Handle
$worker.Id | Set-Content -LiteralPath "$runOutput\worker.pid"

Write-Host "V8.2.2 FORMAT-ROUTED ENSEMBLE. Worker PID: $($worker.Id)."
Write-Host 'The 300 rollout-label shards are reused; no label generation.'
...
if ($status.phase -eq 'COMPLETE') {
    Write-Host 'COMPLETE. Sealed holdout saved; stopped for review.' -ForegroundColor Green
} elseif ($status.phase -eq 'STOP_FOR_REVIEW') {
    Write-Host 'STOPPED AT VALIDATION. V8.1.1 remains champion.' -ForegroundColor Yellow
} else {
    Write-Host "FAILED. Progress retained. Read status.json and $errorLog" -ForegroundColor Red
}
