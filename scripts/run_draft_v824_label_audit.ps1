...
if(-not $Execute){python scripts/draft_v824_label_audit.py;exit $LASTEXITCODE}

$runOutput='E:\NBAFantasyAnalytics\artifacts\draft_ml\v824-label-reliability'
...
$existing=Get-CimInstance Win32_Process|Where-Object{
    ($_.Name -in @('python.exe','pythonw.exe'))-and($_.CommandLine -like '*draft_v824_label_audit.py*--execute*')
}
if($existing){throw "V8.2.4 already running: $($existing.ProcessId -join ', ')"}

$pythonExe=(Get-Command python).Source;$stamp=Get-Date -Format 'yyyyMMdd-HHmmss';$errorLog="$runOutput\error-$stamp.log"
$worker=Start-Process -FilePath $pythonExe -ArgumentList @('-u','scripts/draft_v824_label_audit.py','--execute') `
...
Write-Host "V8.2.4 LABEL RELIABILITY AUDIT. Worker PID: $($worker.Id). No training."
...
if($status.phase-eq'COMPLETE'){Write-Host 'COMPLETE. Reliability decision saved; no model was trained.' -ForegroundColor Green}
else{Write-Host "FAILED. Profile-level progress retained. Read status.json and $errorLog" -ForegroundColor Red}
